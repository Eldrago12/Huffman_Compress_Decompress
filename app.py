...
def decompress():
    ...
    if request.method == "GET":
        ...
        if len(up_file.filename) > 0:
            ...
            print(up_file.filename)
            up_file.save(os.path.join(app.config["FILE_UPLOADS"], filename))
            os.system('./d /Users/sirshakdolai/Downloads/Huffman_Coding-main/uploads/{}'.format(filename))
            f = open('/Users/sirshakdolai/Downloads/Huffman_Coding-main/uploads/{}'.format(filename), 'rb')
            ftype = "-decompressed." + (f.read(int(f.read(1)))).decode("utf-8")
            filename = filename[:filename.index("-",1)]

            while True:
                if '/Users/sirshakdolai/Downloads/Huffman_Coding-main/uploads/{}{}'.format(filename, ftype) in glob.glob('uploads/*-decompressed.*'):
                    os.system('mv /Users/sirshakdolai/Downloads/Huffman_Coding-main/uploads/{}{} /Users/sirshakdolai/Downloads/Huffman_Coding-main/downloads/'.format(filename, ftype))
                    break
